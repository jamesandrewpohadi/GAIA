...
#	set_process_input(true)
	...
	if (listenerNodePath != "" && listenerNodePath!=null):
		listenerNodePath = get_parent().get_parent().get_parent().get_path()
...
#	print(str(event))
	...
	if need2ChangeActivePointer(event):
		if (currentPointerIDX != incomingPointer) and event.is_pressed() && get_parent().get_rect().has_point(event.position):
...
func isReleased(event):
	if event is InputEventScreenTouch && !event.pressed:
		if get_parent().get_rect().has_point(event.position):
			return !event.is_pressed()
...
#func _on_AreaAnalogLeft_gui_input(event):
#	print(str(event))
#	var incomingPointer = extractPointerIdx(event)
#	#print(incomingPointer)
#
